...
The integration sends interactive cards as direct messages from a Feishu application bot for work item field changes, comments, and related activities. Reassignment and state handoffs include the old and new assignees as recipients. Changes to the state-assignees map also notify the previous and new owners of the affected states. The person making the change is excluded from all activity notifications, including when they are a current, previous, or state-specific assignee. Other responsible recipients are still notified. Recipients need an active Plane workspace membership and active membership in the work item's project. Plane identifies their Feishu account automatically using only the mobile synced from SSO. Missing, invalid, or unmatched mobile numbers cause the notification to be skipped with a reason in delivery history. Cards link to work item details, which still require Plane access permission.

## Notification wording and names

Cards are titled **工作项更新提醒 - 工作项标题**. The first block identifies the work item and explicitly describes it as related to the recipient, such as “显示昵称修改了与你有关的工作项” or “显示昵称在与你有关的工作项中发表了评论”.

Actor and assignee names use the member's Plane `display_name`, including previous and new owners in reassignment and state-assignee changes. Names are read from active workspace members' local profiles in the worker and frozen before transport so message retries keep the same content. Missing or blank names display “成员”. Displaying names does not require a mobile number or a Feishu contact-profile lookup. The recipient list still comes from SSO mobile identification and the work-item responsibility rules, excluding the actor.
...
4. Configure the application's contact data permissions and availability/visibility range for all intended recipients. For organization-wide notifications, grant the full intended organization's contact access and application visibility. A messaging scope alone does not grant mobile lookup access, and a valid ID alone does not make a recipient reachable by the bot. Card display names use Plane profiles and do not require Feishu user-info field permissions.
