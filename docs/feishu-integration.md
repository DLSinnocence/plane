...
## Notification wording and names

Cards are titled **工作项更新提醒 - 工作项标题**. The first block identifies the work item and explicitly describes it as related to the recipient, such as “飞书昵称修改了与你有关的工作项” or “飞书昵称在与你有关的工作项中发表了评论”.

Actor and assignee names are resolved from Feishu contact profiles: `nickname` first, then `name`, then `en_name`. MeowAlive/Plane display names are not used as a fallback in the card. Name lookup happens in the worker, and the rendered names are frozen before transport so message retries keep the same content. Missing names or unavailable contact permissions display “成员（飞书昵称不可用）” while retaining the alert; transient provider failures retry. The recipient list still comes only from SSO mobile identification and the work-item responsibility rules.
...
4. For card nicknames, enable **Get basic information in contacts** and the **Obtain user's basic information** field permission (飞书控制台中的“获取通讯录基本信息”和“获取用户基本信息”), or an applicable existing broader contacts permission. The [official user-info documentation](https://open.feishu.cn/document/server-docs/contact-v3/user/get) lists these requirements and the `nickname`, `name`, and `en_name` fields.
5. Configure the application's contact data permissions and availability/visibility range for all intended recipients. For organization-wide notifications, grant the full intended organization's contact access and application visibility. A messaging scope alone does not grant contact lookup access, and a valid ID alone does not make a recipient reachable by the bot.
6. Create and publish a new application version and complete tenant approval. Bot capability, scope changes, and visibility must be effective in the published application.
7. Copy **App ID** and **App Secret** from the application's credentials page. Treat the secret as a credential; do not paste it into issues, screenshots, or delivery reports.
