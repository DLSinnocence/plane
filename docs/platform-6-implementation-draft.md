...
- JPG／JPEG、PNG、GIF、WebP 附件直接显示缩略图，下方不展示文件名或下载按钮；点击图片后放大查看，悬停提示和大图标题保留完整文件名。按原始文件名的末尾扩展名判断，忽略大小写，不从附件行名称或资产 URL 猜测格式。
- 支持正式工作项附件行、非普通工作项的旧附件列表和收件箱卡片；完整详情和快速预览复用相同实现。
- 可视区域内自动加载最高约 192px 的缩略图，图片显示多宽，外层就占多宽，不保留固定空框；屏幕外使用浏览器懒加载。缩略图和大图均使用原生 `<img>` 保持比例与 GIF 动画。
- 附件 **⋯** 菜单和大图顶部保留明确的带图标**下载**入口。只读用户也可下载，替换和删除仍按原有权限控制，空附件行没有下载项。
...
- 非图片附件保留文件名链接、修饰键点击和新标签页行为。文件操作不会冒泡打开上传选择器；图片查看按钮与下载菜单使用独立元素。
...
| `apps/web/core/components/issues/attachment/download.tsx`                                         | 菜单中的原生下载操作，沿用受保护地址，不读取二进制到 JavaScript 内存。                                          |
| `apps/web/core/components/issues/attachment/file-link.tsx`                                        | 图片仅显示带完整名称提示的缩略图按钮；非图片保留文件名链接与修饰键导航，并隔离上传区域事件。                    |
...
- 浏览器测试：图片预览与下载 62 项及附件行 36 项，共 98 项；覆盖无空框大缩略图、无图片下方文字／按钮、菜单和大图下载入口、只读菜单、缩放／平移、重试焦点、窄屏工具栏与真实中文界面截图。
- Web 与 `@plane/ui` 类型检查。
- 共享依赖构建与 Web 生产构建。
- 20 个语言包的新增键与非空文案校验。
