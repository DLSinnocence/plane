...
> 在当前项目建一个“登录偶发白屏”的工作项，优先级高，简要描述问题和期望行为。
...
支持按阶段调整负责人，例如“把开发阶段交给张三，验收阶段交给李四”。助手先查询项目状态、成员与已有配置，再通过本地 `workitem_metadata` 工具读写公共 API；只修改指定阶段，保留其他阶段负责人。当前负责人随阶段自动切换，待规划、已完成、已取消仍固定归创建者；修改配置沿用创建者或管理员权限。该工具也支持读取和设置父需求，传入 `parent: null` 可解除归属。
...
助手在撰写、改写或精简工作项标题与描述前，会加载内置的 `writing-plane-requirements` Skill。描述默认用一小段话说明需求，必要时列出少量期望行为，保留理解需求必需的背景与约束，不自动追加验收标准、实现方案、测试计划或开发记录。用户明确指定的内容和格式优先，例如“补上复现步骤和验收标准”。

用户说“提需求”“建需求”“新增需求”时，助手应同时关联目标项目的“需求”标签，无需再补充“打标签”。助手先查询并复用已有标签，缺少时创建，再将标签 ID 随工作项一起提交并核实结果；用户指定的其他标签也会保留。明确要求不加标签时按用户要求执行。普通任务、草稿和仅改写已有描述不默认增加“需求”标签。

创建工具会拒绝未填写 `labels` 的调用，提示模型先完成标签选择；明确无需标签时使用空数组 `[]`。此校验能阻止遗漏字段，但标签分类与结果核实仍需模型遵循 Skill，真实模型效果需用实际配置验证。

创建前，助手会读取同项目的相关单据，参考标题、正文、标签和父需求归属。客户端需求会查找并核实已有的客户端父需求；归属明确时挂载并回读确认，有歧义时询问，没有合适父需求时保留为独立工作项。仅改写文字会保留已有标签、父需求和负责人。

也可以点名调用：

> 使用 writing-plane-requirements，把下面的需求整理成简洁的工作项描述：……

工具记录会显示“加载 写作规范”及加载状态，展开后可查看技能名和简短结果。加载本身不会修改工作项；请求草稿时助手返回文字，请求创建或更新时才执行对应操作。仅修改状态、负责人等字段时无需加载写作 Skill。
