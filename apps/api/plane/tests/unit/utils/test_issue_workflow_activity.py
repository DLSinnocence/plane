...
def test_configured_handoff_uses_actual_assignments_and_one_alias(alias, actual):
    destination = uuid4()
    issue = SimpleNamespace(pk=uuid4(), state_id=destination, state_assignees={str(destination): []})
...
@pytest.mark.parametrize("configured,transition", [(False, False), (False, True), (True, False)])
def test_unrelated_or_unconfigured_changes_preserve_payload_without_assignment_query(configured, transition):
    destination = uuid4()
