...
The `/?ai-assistant` fixture mounts the production `AIAssistant` sidebar beside an interactive workspace and uses its actual production stylesheet, Markdown renderer, fetch service, and NDJSON parser. Personal model settings still use the real settings component. User and command-palette stores, settings/CSRF HTTP, and model replies are deterministic; a controlled stream permits intermediate-state assertions. English and Chinese labels come from the real locale JSON (`&lang=zh` selects Chinese). No real credentials or workspace writes occur in browser tests; the opt-in API/Live integration test covers the actual Pi/MCP connection separately.
