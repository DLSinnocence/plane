...
function WorkspaceGiteaSettings({ workspaceSlug }: { workspaceSlug: string }) {
  ...
  useEffect(() => {
    if (!data?.enabled || !data?.has_secret) clearSecrets();
  }, [data?.enabled, data?.has_secret]);

  async function run(action: () => Promise<void>, success = "saved", requestGeneration?: number) {
    if (actionLock.current) return;
    const isCurrent = () =>
      mounted.current && (requestGeneration === undefined || requestGeneration === generation.current);
    ...
    try {
      await action();
      if (isCurrent()) setNotice(text(success));
    } catch {
      if (isCurrent()) setError(text("action_error"));
    } finally {
      ...
      {config.error ? (
        ...
        <>
          ...
          <div className="max-w-3xl space-y-2">
            <p className="text-13">{text("generation_help")}</p>
            <div className="flex flex-wrap gap-2">
              <button
                className={buttonClass}
                disabled={busy || !data.enabled || !data.has_secret}
                onClick={() => {
                  if (actionLock.current) return;
                  clearSecrets();
                  ...
                  void run(
                    async () => {
                      const result = await service.generateHooks(workspaceSlug);
            ...
            <p className="text-13 text-secondary">{text("hook_help")}</p>
            <p className="text-13 text-secondary">{text("hook_requirements")}</p>
          </div>
          {hooks && (
            <div className="max-w-3xl space-y-4">
              <p className="text-13">{text("hook_installation")}</p>
              {(["pre_receive", "post_receive"] as const).map((field) => (
                <div key={field} className="space-y-2">
                  <h5 className="text-14 font-medium">{hooks[field].filename}</h5>
                  <p className="text-13 text-secondary">{text(`${field}_help`)}</p>
                  <textarea
                  ...
                  <button
                    ...
                    disabled={busy}
                    onClick={() =>
                      void run(() => navigator.clipboard.writeText(hooks[field].content), "copied", generation.current)
                    }
                  >
                    {text(`copy_${field}`)}
                  </button>
