...
export const CreateUpdateIssueModalBase = observer(function CreateUpdateIssueModalBase(props: IssuesModalProps) {
  ...
  const allowedProjectIdsKey = allowedProjectIds.join(",");
  ...
  }, [data?.project_id, data?.id, data?.sourceIssueId, projectId, isOpen, activeProjectId, allowedProjectIdsKey]);
  ...
  const handleCycleChange = async (previousIssue: Partial<TIssue> | undefined, payload: Partial<TIssue>) => {
    if (!workspaceSlug || !previousIssue?.project_id || !previousIssue?.id) return;
    ...
    if (!("cycle_id" in payload) || isEqual(previousIssue?.cycle_id, payload.cycle_id)) return;

    const slug = workspaceSlug.toString();

    // Removing the cycle
    const currentCycleId = previousIssue?.cycle_id;
    if (currentCycleId && payload.cycle_id === null) {
      await issues.removeIssueFromCycle(slug, previousIssue.project_id, currentCycleId, previousIssue.id);
      fetchCycleDetails(slug, previousIssue.project_id, currentCycleId).catch((error) => {
        console.error(error);
      });
    }
    ...
    if (
      ...
      await addIssueToCycle(previousIssue as TBaseIssue, newCycleId);
    }
  };

  const handleModuleChange = async (previousIssue: Partial<TIssue>, payload: Partial<TIssue>) => {
    if (!workspaceSlug || !previousIssue?.project_id || !previousIssue?.id) return;
    ...
    if (
      ...
      isEqual(previousIssue?.module_ids, payload.module_ids)
    )
      return;

    const updatedModuleIds = xor(previousIssue.module_ids, payload.module_ids);
    const modulesToAdd: string[] = [];
    const modulesToRemove: string[] = [];

    for (const moduleId of updatedModuleIds) {
      if (previousIssue.module_ids?.includes(moduleId)) {
  ...
  const handleUpdateIssue = async (payload: Partial<TIssue>): Promise<TIssue | undefined> => {
    ...
    try {
      ...
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("error"),
        message: errorKey ? t(errorKey) : (error?.error ?? t("issue_could_not_be_updated")),
      });
    }
  };
